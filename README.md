...
3. 양쪽 앱의 조작 메뉴는 화면 맨 아래 한 줄이며, 접으면 오른쪽 아래 `⋮` 버튼 하나만 남습니다. 선생님은 설정에서 명상/공부/휴식, 시작 대기, 촬영 간격, 두 알림 시간과 두 감도를 변경할 수 있습니다. 명상은 0분도 허용합니다.
...
7. 명상·공부 중 10초마다 학생폰에서 전체 원본 JPEG를 임시 생성한 뒤 `주변을 픽셀화한 960px 썸네일`과 `고화질 책 ROI`로 분리하고 원본을 삭제합니다. 썸네일만 자동 전송하며, 고화질 책 ROI는 선생님이 해당 썸네일을 눌렀을 때 요청·전송합니다.
   파일은 Nearby의 실제 전송 완료 뒤에만 전송됨으로 표시합니다. 실패하면 연결 중 2초 간격으로 최초 시도 뒤 최대 3회 재시도하고, 재연결하면 남은 ROI·음성 및 최신 썸네일을 다시 보냅니다.
8. 선생님 화면은 화면 대부분을 180도 회전한 최신 사진에 사용하고 아래에 최근 썸네일 최대 12개를 두 줄로 표시합니다. 메인 사진이나 썸네일을 누르면 해당 시점의 고화질 책 ROI가 상단 `×` 하나만 있는 전체화면으로 열리고 최대 5배 핀치 확대와 이동을 지원합니다.
...
- JVM/Android unit test 65개: 실패 0, 오류 0, skip 0
